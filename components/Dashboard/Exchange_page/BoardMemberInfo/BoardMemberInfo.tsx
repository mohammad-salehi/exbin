...
const BoardMemberTable = ({ SetC3 }: ExchangeInfoProps) => {
  ...
  // ✅ هر بار خواستی یک لیست رو "ایندکس‌دار" کنی از این استفاده کن
  const withIndex = (list: Omit<Person, "index">[]): Person[] =>
    (list ?? []).map((item, idx) => ({
      ...item,
      index: idx, // 0-based
    }));
  ...
  // ✅ fetch اولیه + index از 0
  useEffect(() => {
    GetRequest(process.env.NEXT_PUBLIC_API_URL + `/api/exchanges/${params.id}/board-members`)
      .then((response) => {
        const raw = (response?.result?.content ?? []) as Omit<Person, "index">[];
        setData(withIndex(raw));
        SetC3(true);
      })
      .catch((err) => {
        console.log(err);
  ...
  const deleteMember = async (row: Person) => {
    ...
    DeleteRequest(`${process.env.NEXT_PUBLIC_API_URL}/api/exchanges/${params.id}/board-members/${row.id}`)
      .then(() => {
        ...
        // ✅ بعد از حذف دوباره index ها رو مرتب کن
        setData((prev) => withIndex(prev.filter((p) => p.id !== row.id).map(({ index, ...rest }) => rest)));
  ...
  const handleSave = async () => {
    ...
    PutRequest(`${process.env.NEXT_PUBLIC_API_URL}/api/exchanges/${params.id}/board-members/${editingId}`, payload)
      .then(() => {
        ...
        // ✅ آپدیت ردیف + حفظ index سپس reindex برای اطمینان
        setData((prev) => {
          const next = prev.map((item) => (item.id === editingId ? { ...item, ...form, id: editingId } : item));
          return withIndex(next.map(({ index, ...rest }) => rest));
        });
  ...
  const handleAdd = async () => {
    ...
    const roleValue = BoardmemderRoleTypes.find((item) => item.label === role)?.value ?? "";

    const payload: Omit<Person, "id" | "index"> = {
      name,
      phoneNumber,
      nationalCode,
      role: roleValue,
    ...
    PostRequest(`${process.env.NEXT_PUBLIC_API_URL}/api/exchanges/${params.id}/board-members`, payload)
      .then((res) => {
        ...
        setData((prev) => {
          const rawPrev = prev.map(({ index, ...rest }) => rest);
          const newItem: Omit<Person, "index"> = {
            ...payload,
            id: res?.result?.id ?? String(rawPrev.length + 1),
          } as any;
          return withIndex([...rawPrev, newItem]);
        });
  ...
  const columns: Column<Person>[] = [
    ...
    { header: "ایمیل", accessorKey: "email" },
    {
      ...
      cell: (row: Person) => (
        <div className="flex items-center gap-2 text-titleText dark:text-titleText-dark">
          {/* ✅ آیدی‌ها بر اساس index */}
          <button
            id={`EditBoardMember${row.index}`}
                  ...
                  >
                   ...
                   dark:text-gray-100 appearance-none relative bg-boxColor dark:bg-boxColor-dark"
                      >
                        <span>
                          {form.role !== ""
                            ? BoardmemderRoleTypes.find((item) => item.value === form.role)?.label
                            : "انتخاب"}
                        </span>
                      </Button>
                    </Dropdown.Trigger>
                        ...
                        <Dropdown.Option value={item.value} key={`option${index}`}>
                          {({ selected, active }) => (
                            <MenuItem
                              isActive={active}
                              isSelected={selected}
                              className={`border mt-2 mb-1 rounded-md border-gray-100 dark:border-buttonBorderColor-dark ${form.role === item.value ? "bg-gray-100 border-gray-200 dark:bg-gray-700" : ""
                                }`}
                            >
                        ...
                        <Dropdown.Option value={item.label} key={`option${index}`}>
                          {({ selected, active }) => (
                            <MenuItem
                              isActive={active}
                              isSelected={selected}
                              className={`border mt-2 mb-1 rounded-md border-gray-100 dark:border-buttonBorderColor-dark ${form.role === item.label ? "bg-gray-100 border-gray-200 dark:bg-gray-700" : ""
                                }`}
                            >
                              <MenuItem.Title>{item.label}</MenuItem.Title>
                            </MenuItem>
      ...
      >
        ...
        <div className="fixed inset-0 z-[2147483647] flex items-center justify-center">
          <Modal.Panel className="bg-boxColor dark:bg-bgColor-dark shadow-xl rounded-xl text-titleText dark:text-titleText-dark w-full max-w-md p-6">
            <h3 className="text-lg font-semibold mb-3 text-center">حذف عضو هیئت‌مدیره و سهامداران</h3>
            <p className="text-sm mb-6 text-center leading-relaxed">{`آیا از حذف عضو هیئت‌مدیره و سهامداران مطمئن هستید؟`}</p>
